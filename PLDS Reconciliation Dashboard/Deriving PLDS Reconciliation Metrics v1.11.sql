...
UPDATED BY OLI DUCKER DECEMBER 2023'
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
COHORT OF PATIENTS ALREADY ELIGILBE (TO USE IN METRICS 4 AND 6)  -- Code currently incorrect DO NOT USE
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
...
--select top(1000) * from #MultipleServiceIDs


--select * from [NHSE_Sandbox_CHC].[chc].[vw_reporting_101_ReferralAssessmentOutcome_Current]
--where OrgIDComm = '03N'
--and LocalPatientId = '29259'
--and RecordEndDate is null


--select * from [NHSE_Sandbox_CHC].[chc].[vw_reporting_101_ReferralAssessmentOutcome_Current]
--where OrgIDComm = 'D4U1Y'
--and LocalPatientId = '13653'
--and RecordEndDate is null
...
--ON b.LocalPatientId = a.LocalPatientID AND b.OrgIDComm = a.OrgIDComm
...
--AND b.RecordEndDate IS NULL   -- BRACKETS MISSING!! 
...
--		select top 1000 * from #AlreadyEligible order by OrgIDComm, LocalPatientId 
		
--		select * from [NHSE_Sandbox_CHC].[chc].[vw_reporting_101_ReferralAssessmentOutcome_Current] 
----		where OrgIDComm = '01F' and LocalPatientId = '10029'  -- without brackets
----		where OrgIDComm = '03F' and LocalPatientId = '10034'  -- with brackets
----		where OrgIDComm = '00P' and LocalPatientId = '1900'   -- with brackets
--		where OrgIDComm = '00L' and LocalPatientId = '175308'  -- without brackets
--		AND RecordEndDate IS NULL
